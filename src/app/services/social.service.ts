...
export class SocialService {
  ...
  add = (id: string, userId: any): Promise<void> =>
    this.http.put<void>(`${API_BASE_URL}/users/${id}/followed`, userId).toPromise()
      .catch(({ error: { error } }) => {
        throw new Error(error);
      })

  remove = (id: string, userId: string): Promise<void> =>
    this.http.delete<void>(`${API_BASE_URL}/users/${id}/followed/${userId}`).toPromise()
      .catch(({ error: { error } }) => {
        throw new Error(error);
      })

  block = (id: string, userId: string): Promise<void> =>
    this.http.delete<void>(`${API_BASE_URL}/users/${id}/followers/${userId}`).toPromise()
      .catch(({ error: { error } }) => {
        throw new Error(error);
      })
}
